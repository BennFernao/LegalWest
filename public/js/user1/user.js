...
async function inicializadorDadosDoUser(){
    ...
    function escreverDadosParaModal(user){
        ...
        nomeModalUser.value = nome
        sobrenomeModalUser.value = sobrenome
        emailModalUser.value = email
        telefoneModalUser.value = telefone
        ...
        botaoAtualizarSenhaDoUserModal.addEventListener("click", async ()=>{
            ...
            if(resposta[0] == "erro"){
                ...
                divAlerta.className = "alert alert-danger alerta"
                divAlerta.innerText = resposta[1]

            }else{

                divAlerta.innerText = "Dados atualizados com sucesso"
                divAlerta.className = "alert alert-success alerta"
                setTimeout(()=>{
                    location.reload()
                }, 2000)
            }
