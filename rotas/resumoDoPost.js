...
const ResumoDoPost = require("../database/posts/post")
...
app.post("/addPost", verificadorDeSessaoSuperAdmin, upload.single("imagem") ,async (req, res)=>{

    let {titulo, descricao,   html} = req.body
    let autor = req.session.nomeDoUser + " " + req.session.sobrenomeDoUser

    const urlImagem =  req.file.filename


    
    if(titulo && descricao && urlImagem  && html){
        ...
        try {
                   ...
                   <nav class="navbar navbar-expand-lg   navbar-light " >
                       ...
                       <div class="collapse w-100  " id="navbarCollapse">
                           <div class="d-sm-block d-lg-none">
                               ...
                               <!--Div para a tradução-->
                               <a href="/marcarConsulta" class="btn btn-primary mr-5 ml-4 mb-2" >Marcar Consulta</a>
                       ...
                       <div class="d-none d-lg-flex flex-row justify-content-start align-items-center w-100 " >
                           ...
                           <a href="/marcarConsulta" class="btn btn-primary mr-5" >Marcar Consulta</a>           
            ...
            <!-- Header End -->
                ...
                `
            let tituloOriginal = titulo.toString()    
            titulo = titulo.replace(/\s/g, "") + Date.now()

            fs.writeFile(path.join(__dirname, `../public/blog/${titulo}.html` ), conteudoDaPagina, async(error)=>{
                if(error){
    
                    res.send["erro", "erro ao adicionar"]
                    
                }else{
    
                 
                    const novoResumo = await ResumoDoPost.create({titulo : tituloOriginal,tituloModificado: titulo, descricao, urlImagem, autor, arquivo: titulo})

                    
                    res.sendFile(path.join(__dirname, "..", "public", "blog", `${titulo}.html`))
                }
            }) 
...
try {        
        let todosPosts = await ResumoDoPost.findAll()
        todosPosts = JSON.parse(JSON.stringify(todosPosts)) 
...
app.get("/showSomePosts", async (req, res)=>{
...
app.get("/deletePost/:id", verificadorDeSessaoSuperAdmin , async (req, res)=>{
    ...
    const id = req.params.id
     ...
     try {
        let post  = await ResumoDoPost.findByPk(id)

        if(post){

            let {tituloModificado, urlImagem} = post.toJSON()

            
            fs.unlink(path.join(__dirname, "..", "public", "blog", `${tituloModificado}.html`), (err)=>{
                if(err){
                    console.log(err)
                    res.send(["erro", "falha ao deletar o post 1"])
                }else{

                    fs.unlink(path.join(__dirname, "..", "public", "img", `${urlImagem}`), async (err)=>{
                        if(err){
                            res.send(["erro", "falha ao deletar o post 2"])
                        }else{

                            const postApagado = await ResumoDoPost.destroy({where:{
                                id
                            }})

                            console.log(postApagado)
                            res.send(["sucesso", "post deletado com sucesso"])
            ...
            res.send(["erro", "erro ao deletar 4"])
        }
            ...
            console.log(error)
            res.send(["erro", "erro ao deletar 3"])
